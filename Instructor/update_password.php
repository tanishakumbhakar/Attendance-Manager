 ...
 {
 $op=md5($op);
	$que=mysqli_query($con,"select * from instructor where email='".$_SESSION['staff']."' and pass='$op'");
	...
	$row= mysqli_num_rows($que);
	 ...
	 if($row)
	 {
		...
		{
		$np=md5($np);
		mysqli_query($con,"update instructor set pass='$np' where email='".$_SESSION['staff']."'");
