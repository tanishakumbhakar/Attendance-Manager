 ...
 {
 $op=md5($op);
	$que=mysqli_query($con,"select * from student where email='".$_SESSION['student']."' and pass='$op'");
	...
	$row= mysqli_num_rows($que);
	 ...
	 if($row)
	 {
		...
		{
		$np=md5($np);
		mysqli_query($con,"update student set pass='$np' where email='".$_SESSION['student']."'");
